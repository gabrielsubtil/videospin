#!/usr/bin/env python3
"""
Lint Runner - Unified linting and type checking (Patched for PATH issues)
...
def detect_project_type(project_path: Path) -> dict:
    ...
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists():
        ...
        # Check for ruff (Use sys.executable -m to avoid PATH issues)
...
def main():
    ...
    for linter in project_info["linters"]:
        ...
        if result["passed"]:
            ...
            print(f"  Output: {result['output'][:500]}") # Show output
            if result["error"]:
